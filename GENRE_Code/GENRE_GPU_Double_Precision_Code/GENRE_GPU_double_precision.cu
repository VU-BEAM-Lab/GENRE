...
// Description of GENRE_GPU_double_precision.cu: 
// This file contains the MEX-interface that calls the C/CUDA code for 
// performing the computations for GENRE using double precision
...
#include <string.h>
#include <math.h>
#include "GPU_kernels_double_precision.cu"
#include "mex.h"
#define DEVICE_ID 0
...
static double * y_d;                            // Stores the datasets that are being fit with models
static double * residual_y_d;                   // Stores the residual values that are obtained during each fit
static double * y_std_d;                        // Stores the standard deviations for each portion of the y_d array (one portion corresponds to one fit)
static double * standardized_lambda_values_d;   // Stores the standardized lambda values for each portion of the y_d array (one portion corresponds to one fit)
...
static double * X_matrix_d;                     // Stores all of the model matrices
...
static double * B_d;                            // Stores the predictor coefficient values that are obtained from each fit
...
static double * alpha_values_d;                 // Stores the alpha values that are used in elastic-net regularization         
static double * tolerance_values_d;             // Stores the tolerance values that are used for cyclic coordinate descent
static double * max_iterations_values_d;        // Stores the maximum number of iterations values that are used for cyclic coordinate descent
...
if (!initialized) {
   ...
   X_matrix_h = (double*)mxGetData(prhs[5]);
   B_thread_stride_h = (double*)mxGetData(prhs[6]);
   intercept_flag_h = (double*)mxGetData(prhs[7]);

   // Allocate the GPU device arrays
   cudaMalloc(&y_d, total_num_y_observations * sizeof(double));
   cudaMalloc(&residual_y_d, total_num_y_observations * sizeof(double));
   cudaMalloc(&y_std_d, num_fits * sizeof(double));
   ...
   cudaMalloc(&X_matrix_d, total_num_X_matrix_values * sizeof(double));
   cudaMalloc(&B_d, total_num_B_values * sizeof(double));
   cudaMalloc(&B_thread_stride_d, num_fits * sizeof(double));
   cudaMalloc(&alpha_values_d, num_fits * sizeof(double));
   cudaMalloc(&standardized_lambda_values_d, num_fits * sizeof(double));
   cudaMalloc(&tolerance_values_d, num_fits * sizeof(double));
   cudaMalloc(&max_iterations_values_d, num_fits * sizeof(double));
   cudaMalloc(&intercept_flag_d, num_fits * sizeof(double));
   cudaMalloc(&scaling_factors_d, total_num_B_values * sizeof(double));
   cudaMalloc(&mean_X_matrix_d, total_num_B_values * sizeof(double));
   cudaMalloc(&model_fit_flag_d, num_fits * sizeof(double));
   ...
   cudaMemcpy(X_matrix_d, X_matrix_h, total_num_X_matrix_values * sizeof(double), cudaMemcpyHostToDevice);
   cudaMemcpy(B_thread_stride_d, B_thread_stride_h, num_fits * sizeof(double), cudaMemcpyHostToDevice);
   cudaMemcpy(intercept_flag_d, intercept_flag_h, num_fits * sizeof(double), cudaMemcpyHostToDevice);
...
// Declare the pointers to the host arrays
double * alpha_values_h;
double * lambda_values_h;
double * tolerance_values_h;
double * max_iterations_values_h;

// Obtain the parameter arrays
alpha_values_h = (double*)mxGetData(prhs[8]);
lambda_values_h = (double*)mxGetData(prhs[9]);
tolerance_values_h = (double*)mxGetData(prhs[10]);
max_iterations_values_h = (double*)mxGetData(prhs[11]);

// Transfer the data from the host arrays to the GPU device arrays
cudaMemcpy(alpha_values_d, alpha_values_h, num_fits * sizeof(double), cudaMemcpyHostToDevice);
cudaMemcpy(standardized_lambda_values_d, lambda_values_h, num_fits * sizeof(double), cudaMemcpyHostToDevice);
cudaMemcpy(tolerance_values_d, tolerance_values_h, num_fits * sizeof(double), cudaMemcpyHostToDevice);
cudaMemcpy(max_iterations_values_d, max_iterations_values_h, num_fits * sizeof(double), cudaMemcpyHostToDevice);
        
// Declare the pointer to the host array
double * y_h;

// Obtain the input data array
y_h = (double*)mxGetData(prhs[12]);

// Transfer the input data from the host array to the GPU device array
cudaMemcpy(y_d, y_h, total_num_y_observations * sizeof(double), cudaMemcpyHostToDevice);

// Set the predictor coefficient values to 0
cudaMemset(B_d, 0, total_num_B_values * sizeof(double));

// Set the model fit flags to 0
cudaMemset(model_fit_flag_d, 0, num_fits * sizeof(double));
...
int num_blocks = (int)ceil((double)num_fits / (double)num_threads_per_block);
...
if (shared_memory_flag == 1) {
   model_fit_shared_memory<<<MODEL_FIT_GRID_SIZE, MODEL_FIT_BLOCK_SIZE, num_threads_per_block * max_num_observations * sizeof(double)>>>(B_d, B_thread_stride_d, model_fit_flag_d, X_matrix_d, X_matrix_thread_stride_d, observation_thread_stride_d, residual_y_d, y_std_d, standardized_lambda_values_d, num_observations_d, num_predictors_d, alpha_values_d, tolerance_values_d, max_iterations_values_d, intercept_flag_d, transformation_flag, num_threads_per_block, num_blocks, num_threads_last_block);
} else {
...
plhs[0] = mxCreateNumericMatrix(total_num_B_values, 1, mxDOUBLE_CLASS, mxREAL);
B_h = (double*)mxGetData(plhs[0]);

// Transfer the output
cudaMemcpy(B_h, B_d, total_num_B_values * sizeof(double), cudaMemcpyDeviceToHost);
