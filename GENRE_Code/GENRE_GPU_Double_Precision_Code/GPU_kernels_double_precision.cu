...
// Description of GPU_kernels_double_precision.cu: 
// This file contains the CUDA code that allows for performing the computations
// for GENRE on a GPU using double precision
        
        
// Define the GPU kernel that performs predictor normalization
__global__ void predictor_normalization(double * X_matrix_d, double * scaling_factors_d, double * X_matrix_thread_stride_d, double * B_thread_stride_d, double * num_observations_d, double * num_predictors_d, double * intercept_flag_d, int num_threads_per_block, int num_blocks, int num_threads_last_block) {
...
if (block_thread_ind < num_threads_per_block_2) {
   ...
   for (int predictor_column = start_ind; predictor_column < num_predictors; predictor_column++) {
       ...
       for (int observation_row = 0; observation_row < num_observations; observation_row++) {
           double X_value = X_matrix_d[X_thread_stride + (predictor_column * num_observations) + observation_row];
           sum_squared = sum_squared + (X_value * X_value);
       }

       // Calculate the square root of the sum of the square of the predictor column
       double square_root_sum_squared = sqrt(sum_squared);
   ...
   if (intercept_flag == 1) {
      scaling_factors_d[predictor_thread_stride] = (double)1.0;
   }
}
...
__global__ void predictor_standardization(double * X_matrix_d, double * scaling_factors_d, double * mean_X_matrix_d, double * X_matrix_thread_stride_d, double * B_thread_stride_d, double * num_observations_d, double * num_predictors_d, double * intercept_flag_d, int num_threads_per_block, int num_blocks, int num_threads_last_block) {
...
if (block_thread_ind < num_threads_per_block_2) {
   ...
   for (int predictor_column = start_ind; predictor_column < num_predictors; predictor_column++) {
       ...
       double mean_value = sum_value / (double)num_observations;
       ...
       // Normalize the predictor column by dividing each observation in the predictor column by the square root of the sum of the square of the predictor column
       for (int observation_row = 0; observation_row < num_observations; observation_row++) {
           double X_value_demeaned = X_matrix_d[X_thread_stride + (predictor_column * num_observations) + observation_row] - mean_value;
       ...
       double std = sqrt(sum_squared / (double)num_observations);
   ...
   if (intercept_flag == 1) {
      scaling_factors_d[predictor_thread_stride] = (double)1.0;
      mean_X_matrix_d[predictor_thread_stride] = (double)1.0;
   }
}
...
__global__ void model_fit_preparation(double * y_d, double * residual_y_d, double * model_fit_flag_d, double * y_std_d, double * standardized_lambda_values_d, double * num_observations_d, double * observation_thread_stride_d, int num_threads_per_block, int num_blocks, int num_threads_last_block) {
...
if (block_thread_ind < num_threads_per_block_2) {
   ...
   for (int observation = 0; observation < num_observations; observation++) {
       double value = y_d[observation_thread_stride + observation];
       sum_value += value;
   }
   ...
   for (int observation = 0; observation < num_observations; observation++) {
       double value_2 = y_d[observation_thread_stride + observation];
   ...
   if (std != (double)0.0) {
       ...
       model_fit_flag_d[fit_ind] = (double)1.0;
       ...
       for (int observation = 0; observation < num_observations; observation++) {
           double standardized_value = y_d[observation_thread_stride + observation] / std;
...
if (block_thread_ind < num_threads_per_block_2) { 
   ...
   if (model_fit_flag == 1) {
      ...
      double alpha = alpha_values_d[fit_ind];

      // Obtain the standardized lambda value for the fit
      double lambda = standardized_lambda_values_d[fit_ind];

      // Obtain the tolerance value for the fit
      double tolerance = tolerance_values_d[fit_ind];
      ...
      while (global_max_change >= tolerance && iteration_count < max_iterations) {
            ...
            for (int j = 0; j < num_predictors; j++) {
                ...
                double B_j = B_d[predictor_thread_stride + j];
                ...
                if (B_j != (double)0.0) {
                   for (int observation_row = 0; observation_row < num_observations; observation_row++) {
                       ...
                       double X_value = X_matrix_d[X_thread_stride + (j * num_observations) + observation_row];
 
                       // Remove the contribution of the current predictor from the current residual value
                       double residual_y_value = residual_y_d[observation_thread_stride + observation_row] + (X_value * B_j);
                   ...
                   for (int observation_row = 0; observation_row < num_observations; observation_row++) {
                       ...
                       double residual_y_value = residual_y_d[observation_thread_stride + observation_row];
                ...
                if (transformation_flag == 2 || transformation_flag == 4) {
                    if (intercept_flag == 1 && j > 0) {
                       change = ((double)1.0 / (double)num_observations) * change;
            ...
            for (int j = 0; j < num_predictors; j++) {
                ...
                if (B_j != (double)0.0) {
                   for (int observation_row = 0; observation_row < num_observations; observation_row++) {
                       ...
                       // Remove the contribution of the current predictor from the current residual value
                       double residual_y_value = sdata[(observation_row * num_threads_per_block) + block_thread_ind] + (X_value * B_j);
                   ...
                   for (int observation_row = 0; observation_row < num_observations; observation_row++) {
                       ...
                       // Obtain the residual value (this is essentially the residual value leaving the current predictor out because the predictor coefficient value is zero) 
                       double residual_y_value = sdata[(observation_row * num_threads_per_block) + block_thread_ind];
...
__global__ void predictor_coefficient_unnormalization(double * B_d, double * B_thread_stride_d, double * model_fit_flag_d, double * X_matrix_d, double * X_matrix_thread_stride_d, double * scaling_factors_d, double * num_predictors_d, double * intercept_flag_d, int num_threads_per_block, int num_blocks, int num_threads_last_block) {
...
__global__ void predictor_coefficient_unstandardization(double * B_d, double * B_thread_stride_d, double * model_fit_flag_d, double * X_matrix_d, double * X_matrix_thread_stride_d, double * scaling_factors_d, double * mean_X_matrix_d, double * num_predictors_d, double * intercept_flag_d, int num_threads_per_block, int num_blocks, int num_threads_last_block) {
...
if (block_thread_ind < num_threads_per_block_2) { 
   ...
   if (model_fit_flag == 1) {
      ...
      // Unstandardize the predictor coefficients
      for (int predictor_column = start_ind; predictor_column < num_predictors; predictor_column++) {
          double B_unstandardized = B_d[predictor_thread_stride + predictor_column] / scaling_factors_d[predictor_thread_stride + predictor_column];
