...
In this equation, N represents the number of observations, P represents the number of predictors, **X** is the model matrix containing P predictors with N observations each, **y** is the observed data vector of length N to which the model matrix is being fit, **&beta;** is the vector of model coefficients, &lambda; is a scaling factor for the amount of regularization that is applied, and &alpha; is a factor that wieghts the L1-regularization and the L2-regularization.
