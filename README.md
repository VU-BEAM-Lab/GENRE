...
4. As previously stated, each model fit is supplied a lambda value in GEN, so the coefficients are not calculated for a path of lambda
   ...
   is to follow the method described in 3 above. For example, the ```GEN.m``` script can be modified to call either the ```GEN_GPU_single_precision``` MEX-file or the ```GEN_GPU_double_precision``` MEX-file in a for loop. In each iteration of the for loop, a
   new ```lambda_values_h``` vector can be supplied.
