...
4. As previously stated, each model fit is supplied a lambda value in ```GENRE```, so the coefficients are not calculated for a path of 
   ...
   script can be modified to call either the ```GEN_GPU_single_precision``` MEX-file or the ```GEN_GPU_double_precision``` MEX-file in a for 
   loop. In each iteration of the for loop, the ```lambda_values_h``` vector can be modified to have new values and then be supplied to the 
   MEX-files.
