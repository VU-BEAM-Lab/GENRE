...
## Input Data Format
As previously stated, GEN allows for many models to run in parallel on the GPU. The data for each model fit needs to be saved as a ```.mat``` file. For example, if there are 100 model fits that need to be performed, then there should be 100 ```.mat``` files. Each file should contain 3 variables that are called ```X```, ```y```, and ```intercept_flag```.
